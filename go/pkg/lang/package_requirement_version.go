...
import (
	"github.com/mojo-lang/core/go/pkg/core"
	"strconv"
	"strings"
)
...
func NewPackageRequirementVersion(str string) *Package_Requirement_Version {
	...
	v.Type = Package_Requirement_Version_caret

	if strings.Contains(str, "*") {
		v.Type = Package_Requirement_Version_wildcard
		...
		case '~':
			v.Type = Package_Requirement_Version_tilde
			v.Range = parseTilde(str[1:])
		case '>', '<', '=':
			v.Type = Package_Requirement_Version_comparison
...
func (v version) toVersion() *core.Version {
	...
	values := []*int32{&version.Major, &version.Minor, &version.Patch}
	for i, segment := range v {
...
func parseCaret(v string) *core.VersionRange {
	...
	if i := version.firstNonZero(); i >= 0 && i < 3 {
		values := []*int32{&vr.Max.Major, &vr.Max.Minor, &vr.Max.Patch}
		*values[i]++
		for j := i + 1; j < 3; j++ {
			*values[j] = 0
		}
